...
let banana = { x: Math.floor(Math.random() * (gameArea.clientWidth / 20)) * 20, y: Math.floor(Math.random() * (gameArea.clientHeight / 20)) * 20 };
let score = 0;
let direction = 'RIGHT';
let gameInterval;

function drawTurtle() {
    gameArea.innerHTML = ''; // إعادة تعيين منطقة اللعبة
...
function placeBanana() {
    banana.x = Math.floor(Math.random() * (gameArea.clientWidth / 20)) * 20;
    banana.y = Math.floor(Math.random() * (gameArea.clientHeight / 20)) * 20;
}
